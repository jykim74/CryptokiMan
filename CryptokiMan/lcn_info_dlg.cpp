...
void LCNInfoDlg::initialize()
{
    ...
    if( manApplet->isLicense() )
    {
        ...
        ret = JS_LCN_IsValid( &sLicenseInfo, LICENSE_PRODUCT_CRYPTOKIMAN_NAME, sLicenseInfo.sSID, time(NULL) );
...
bool LCNInfoDlg::isValidLCN( const BIN *pLCN, const char *pSID )
{
    ...
    ret = JS_LCN_IsValid( &sInfo, LICENSE_PRODUCT_CRYPTOKIMAN_NAME, pSID, time(NULL) );
...
void LCNInfoDlg::clickGet()
{
    ...
    ret = JS_LCN_IsValid( &sInfo, LICENSE_PRODUCT_CRYPTOKIMAN_NAME, sInfo.sSID, time(NULL) );
    if( ret != LICENSE_VALID )
    {
