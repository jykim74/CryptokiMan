...
int CSRInfoDlg::saveAsPEM( const BIN *pData )
{
    ...
    if( fileName.length() > 0 )
    {
        ...
        if( ret > 0 )
        {
            manApplet->messageBox( tr( "The CSR was saved in PEM format." ), this );
        }
    }
...
{
    ExportDlg exportDlg;
    exportDlg.setName( QString( "%1_csr" ).arg( mKIDText->text() ) );
...
void CSRInfoDlg::clickVerifyCSR()
{
    ...
    if( ret == 1 )
        manApplet->messageBox( tr( "CSR verification successful" ), this );
    else
        manApplet->messageBox( tr( "CSR verification failed [%1]" ).arg(ret), this );
}
