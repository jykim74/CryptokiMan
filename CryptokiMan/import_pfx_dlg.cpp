...
void ImportPFXDlg::accept()
{
    ...
    else if( key_type == JS_PKI_KEY_TYPE_ML_DSA || key_type == JS_PKI_KEY_TYPE_ML_KEM || key_type == JS_PKI_KEY_TYPE_SLH_DSA )
    {
        rv = JS_PKI_getRawKeyVal( &binPri, &rawKeyVal );
        if( rv == 0 )
        {
...
int ImportPFXDlg::createPQCPublicKey( JRawKeyVal *pRawKeyVal )
{
    int rv = -1;
    CK_SESSION_HANDLE hSession = slot_info_.getSessionHandle();
...
int ImportPFXDlg::createPQCPrivateKey( JRawKeyVal *pRawKeyVal )
{
    int rv = -1;
    CK_SESSION_HANDLE hSession = slot_info_.getSessionHandle();
